...
import { publicApi } from '@/lib/apiBase';
...
export const authFx = appD.createEffect(async (pair: KeyPair) => {
    ...
    const { data } = await publicApi.post<{ ok: boolean; token: string }>(
        '/auth',
        {
...
const loginWithPasswordFx = appD.createEffect(
    async ({ account, password }: { account: StoredAccount; password: string }) => {
        try {
            await sodium.ready;
...
const saveAccountFx = appD.createEffect(
    async ({ seed, name, password }: { seed: string; name: string; password: string }) => {
        try {
            ...
            const savedAccounts: StoredAccount[] = JSON.parse(
                localStorage.getItem('securechat_accounts') || '[]',
            );
...
export const $keyPair = appD
    .createStore<KeyPair | null>(null)
...
export const $pk = combine($keyPair, (keyPair) =>
    keyPair ? sodium.to_hex(keyPair.publicKey) : null,
);
